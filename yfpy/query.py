...
class YahooFantasySportsQuery(object):
    ...
    def __init__(self, auth_dir: Union[Path, str], league_id: str, unique_id: str, 
        ...
        if not self.offline:
            self._authenticate()
            st.session_state['token_file_path'] = self.token_file_path
    ...
    def _authenticate(self) -> None:
        ...
        # Create a dictionary to hold the token information
        token_info = {
        ...
        # Save the token information to a unique JSON file based on the session's unique ID
        token_file_name = f"token_{self.unique_id}.json"
        token_file_path = os.path.join(os.getcwd(), token_file_name)
        self.token_file_path = token_file_path

        with open(token_file_path, "w") as f:
            json.dump(token_info, f)

        # Initialize the OAuth2 session using the saved token file
    ...
    def cleanup(self):
        ...
        if hasattr(self, 'token_file_path') and os.path.exists(self.token_file_path):
            os.remove(self.token_file_path)
            logger.info(f"Token file {self.token_file_path} has been removed.")
